...
def format_master_md(design_system: dict) -> str:
    ...
    if anti_patterns:
        ...
        for anti in anti_list:
            if anti:
                lines.append(f"- {anti}")
    lines.append("- Emojis as icons (use SVG: Heroicons, Lucide)")
    lines.append("- Missing cursor:pointer on clickable elements")
    lines.append("- Low contrast text (keep 4.5:1 minimum)")
    lines.append("- Instant state changes (use 150-300ms transitions)")
...
def format_page_override_md(design_system: dict, page_name: str, page_query: str = None) -> str:
    ...
    for title, key in [("Layout Overrides", "layout"), ("Spacing Overrides", "spacing"),
